...
if __name__ == '__main__':
    with open("all-protein_id.txt", "r") as f: 
        protein_id = f.read()  
        # print(protein_id)

    with open("protein_sequences_all.txt", "r") as f:  
        protein_seq = f.read()  
        # print(protein_seq)
    ...
    with open("node2vec_result_only_vec.txt", "r") as f:
    ...
    with CustomObjectScope(
        ...
        model = load_model('lr(reduce 1e-5 per epoch)+Adam+CA.h5')  # 选取自己的.h5模型名称


    with open("p-p.txt", "r") as f:  # 打开待预测蛋白质文件
        predicting = f.read()  
        print(protein_seq)
    ...
    if(len(predicting_list)>1):
        for A in range(0, len(predicting_list)):
            print("第"+str(A)+"对")
            pro=predicting_list[A].split(" ")
            #蛋白质对格式为 9606.ENSP00000000233 9606.ENSP00000409666 样式时用这个
            # pro1=pro[0].split(".")[1]
            # pro2=pro[1].split(".")[1]


            # 蛋白质对格式为 ENSP00000000233 ENSP00000409666 样式时用这个
            pro1 = pro[0]
            pro2 = pro[1]

            # print(A)
            if(pro1 in protein_id_list):
                seqA =protein_seq_list[protein_id_list.index(pro1)]
                if (len(seqA) > 1800):
                    #print("大于1800")
                    pass
                if(len(seqA)< 1800):
                    if ((pro2 in protein_id_list) and pro2!=pro1 and len(protein_seq_list[protein_id_list.index(pro2)])<1800):
                        seqB=protein_seq_list[protein_id_list.index(pro2)]
                        ...
                        if(predict[0]==1):
                            with open("PP_interaction_difference_PPI.txt", "a") as f:
                                f.write(protein_id_list[protein_seq_list.index(seqA)])
                                f.write("\t")
                                f.write(protein_id_list[protein_seq_list.index(seqB)])
                                f.write("\n")
                        if (predict[0] == 0):
                            with open("PP_noninteraction_difference_PPI.txt", "a") as f:
                                f.write(protein_id_list[protein_seq_list.index(seqA)])
                                f.write("\t")
                                f.write(protein_id_list[protein_seq_list.index(seqB)])
                                f.write("\n")
